    ...
    @if (loading()) {
              ...
              @for (asset of day.assets; track asset.id) {
                @if (asset.type === 'photo') {
                  <div class="group relative overflow-hidden rounded-lg bg-navy-mid cursor-zoom-in"
                    ...
                    <img [src]="asset.url" [alt]="asset.caption || day.dayName" referrerpolicy="no-referrer"
                    ...
                    @if (asset.caption) {
                      <p class="absolute inset-x-0 bottom-0 px-2.5 py-1.5 text-xs text-white bg-gradient-to-t from-navy-deepest/90 to-transparent opacity-0 group-hover:opacity-100 transition line-clamp-2">
                        {{ asset.caption }}
                      </p>
                    }
                  </div>
      ...
      @if (lightboxAsset(); as asset) {
        <div class="fixed inset-0 z-50 bg-navy-deepest/95 flex items-center justify-center p-4"
          ...
          <div class="flex flex-col items-center max-h-[90vh]" (click)="$event.stopPropagation()">
            <img [src]="asset.url" [alt]="asset.caption || ''" referrerpolicy="no-referrer"
                 class="max-h-[80vh] max-w-[90vw] object-contain rounded-lg shadow-2xl">
            @if (asset.caption) {
              <p class="mt-3 text-sm text-text-light text-center max-w-[90vw]">{{ asset.caption }}</p>
            }
          </div>
        </div>
      }
